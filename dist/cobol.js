...
Also used for EXEC CICS and EXEC DLI in mainframe environments.`}};e.languages.registerHoverProvider("cobol",{provideHover:function(R,t){let n=R.getWordAtPosition(t);if(!n)return null;let i=n.word.toUpperCase();if(C[i])return{range:new e.Range(t.lineNumber,n.startColumn,t.lineNumber,n.endColumn),contents:[{value:"**"+C[i].desc+"**"},{value:"```\n"+C[i].detail+"\n```"}]};if(["ABS","ACOS","ANNUITY","ASIN","ATAN","CHAR","COMBINED-DATETIME","COS","CURRENT-DATE","DATE-OF-INTEGER","DATE-TO-YYYYMMDD","DAY-OF-INTEGER","DAY-TO-YYYYDDD","DISPLAY-OF","E","EXP","EXP10","FACTORIAL","INTEGER","INTEGER-OF-DATE","INTEGER-OF-DAY","INTEGER-PART","LENGTH","LOG","LOG10","LOWER-CASE","MAX","MEAN","MEDIAN","MIDRANGE","MIN","MOD","NATIONAL-OF","NUMVAL","NUMVAL-C","ORD","ORD-MAX","ORD-MIN","PI","PRESENT-VALUE","RANDOM","RANGE","REM","REVERSE","SIN","SQRT","STANDARD-DEVIATION","SUM","TAN","TRIM","UPPER-CASE","VARIANCE","WHEN-COMPILED","YEAR-TO-YYYY"].indexOf(i)!==-1)return{range:new e.Range(t.lineNumber,n.startColumn,t.lineNumber,n.endColumn),contents:[{value:"**FUNCTION "+i+"**"},{value:"```\nCOBOL intrinsic function.\nUsage: FUNCTION "+i+"(arguments)\n```"}]};let T=R.getValue().split(`
`),r=new RegExp("\\b"+n.word.replace(/[-]/g,"\\-")+"\\b","i");for(let a=0;a<T.length;a++){let A=T[a].trim(),O=A.match(/^(\d{1,2})\s+([\w-]+)\s*(PIC|PICTURE|VALUE|OCCURS|REDEFINES)?/i);if(O&&O[2].toUpperCase()===i)return{range:new e.Range(t.lineNumber,n.startColumn,t.lineNumber,n.endColumn),contents:[{value:"**Data Item: "+O[2]+"** (Level "+O[1]+")"},{value:"```cobol\n"+A+"\n```"},{value:"*Defined at line "+(a+1)+"*"}]};let l=A.match(/^([A-Z][A-Z0-9-]+)\s*\.\s*$/i);if(l&&l[1].toUpperCase()===i&&a+1!==t.lineNumber)return{range:new e.Range(t.lineNumber,n.startColumn,t.lineNumber,n.endColumn),contents:[{value:"**Paragraph: "+l[1]+"**"},{value:"*Defined at line "+(a+1)+"*"}]}}return null}}),e.languages.registerDefinitionProvider("cobol",{provideDefinition:function(R,t){let n=R.getWordAtPosition(t);if(!n)return null;let i=n.word.toUpperCase(),E=R.getValue().split(`
`);for(let T=0;T<E.length;T++){let r=E[T].trim(),a=r.match(/^(\d{1,2})\s+([\w-]+)/i);if(a&&a[2].toUpperCase()===i){let O=E[T].indexOf(a[2])+1;return{uri:R.uri,range:new e.Range(T+1,O,T+1,O+a[2].length)}}let s=r.match(/^([A-Z][A-Z0-9-]+)\s*\.\s*$/i);if(s&&s[1].toUpperCase()===i){let O=E[T].indexOf(s[1])+1;return{uri:R.uri,range:new e.Range(T+1,O,T+1,O+s[1].length)}}let A=r.match(/^([A-Z][A-Z0-9-]+)\s+SECTION\s*\./i);if(A&&A[1].toUpperCase()===i){let O=E[T].indexOf(A[1])+1;return{uri:R.uri,range:new e.Range(T+1,O,T+1,O+A[1].length)}}}return null}}),e.languages.registerDocumentSymbolProvider("cobol",{provideDocumentSymbols:function(R){let t=[],n=R.getLinesContent();for(let i=0;i<n.length;i++){let N=n[i].trim(),E=i+1,T=N.match(/^(IDENTIFICATION|ENVIRONMENT|DATA|PROCEDURE)\s+DIVISION/i);T&&t.push({name:T[0],kind:e.languages.SymbolKind.Module,range:new e.Range(E,1,E,n[i].length+1),selectionRange:new e.Range(E,1,E,n[i].length+1)});let r=N.match(/^([A-Z][A-Z0-9-]*)\s+SECTION\s*\./i);r&&t.push({name:r[1]+" SECTION",kind:e.languages.SymbolKind.Namespace,range:new e.Range(E,1,E,n[i].length+1),selectionRange:new e.Range(E,1,E,n[i].length+1)});let a=N.match(/^([A-Z][A-Z0-9-]+)\s*\.\s*$/i);a&&!N.match(/DIVISION|SECTION/i)&&t.push({name:a[1],kind:e.languages.SymbolKind.Function,range:new e.Range(E,1,E,n[i].length+1),selectionRange:new e.Range(E,1,E,n[i].length+1)});let s=N.match(/^(01|1)\s+([\w-]+)/i);s&&s[2].toUpperCase()!=="FILLER"&&t.push({name:s[2],kind:e.languages.SymbolKind.Variable,range:new e.Range(E,1,E,n[i].length+1),selectionRange:new e.Range(E,1,E,n[i].length+1)});let A=N.match(/^(FD|SD)\s+([\w-]+)/i);A&&t.push({name:A[1]+" "+A[2],kind:e.languages.SymbolKind.File,range:new e.Range(E,1,E,n[i].length+1),selectionRange:new e.Range(E,1,E,n[i].length+1)})}return t}}),e.languages.registerCompletionItemProvider("cobol",{triggerCharacters:[" ",".","-"],provideCompletionItems:function(R,t){let n=R.getValueInRange({startLineNumber:t.lineNumber,startColumn:1,endLineNumber:t.lineNumber,endColumn:t.column}),i=R.getWordUntilPosition(t),N={startLineNumber:t.lineNumber,startColumn:i.startColumn,endLineNumber:t.lineNumber,endColumn:i.endColumn},E=[];[{label:"program-skeleton",detail:"Complete COBOL program skeleton",insertText:["IDENTIFICATION DIVISION.","PROGRAM-ID. ${1:PROGRAM-NAME}.","","ENVIRONMENT DIVISION.","CONFIGURATION SECTION.","SOURCE-COMPUTER. ${2:IBM-370}.","OBJECT-COMPUTER. ${2:IBM-370}.","","DATA DIVISION.","WORKING-STORAGE SECTION.","01 ${3:WS-VARIABLES}.","   05 ${4:WS-ITEM}    PIC ${5:X(10)} VALUE ${6:SPACES}.","","PROCEDURE DIVISION.","${7:MAIN-PARA}.",'    ${8:DISPLAY "Hello, COBOL!"}',"    STOP RUN."].join(`
...
`,detail:"Section header",range:N,sortText:"1_"+I})});var r=["ACCEPT","ADD","ALTER","CALL","CANCEL","CLOSE","COMPUTE","CONTINUE","DELETE","DISPLAY","DIVIDE","ENTRY","EVALUATE","EXIT","GO TO","GOBACK","IF","INITIALIZE","INSPECT","INVOKE","MERGE","MOVE","MULTIPLY","OPEN","PERFORM","READ","RELEASE","RETURN","REWRITE","SEARCH","SET","SORT","START","STOP RUN","STRING","SUBTRACT","UNSTRING","WRITE"];r.forEach(function(I){E.push({label:I,kind:e.languages.CompletionItemKind.Keyword,insertText:I+" ",detail:"COBOL verb",documentation:C[I.split(" ")[0]]?C[I.split(" ")[0]].detail:"",range:N,sortText:"2_"+I})});var a=["END-IF","END-EVALUATE","END-PERFORM","END-READ","END-WRITE","END-COMPUTE","END-ADD","END-SUBTRACT","END-MULTIPLY","END-DIVIDE","END-CALL","END-STRING","END-UNSTRING","END-SEARCH","END-DELETE","END-RETURN","END-REWRITE","END-START","END-EXEC"];a.forEach(function(I){E.push({label:I,kind:e.languages.CompletionItemKind.Keyword,insertText:I,detail:"Scope terminator",range:N,sortText:"2_"+I})});var s=["ADVANCING","AFTER","ALL","ALSO","AND","ARE","ASCENDING","ASSIGN","AT","BEFORE","BINARY","BLANK","BLOCK","BY","CHARACTER","CHARACTERS","CLASS","COMP","COMP-1","COMP-2","COMP-3","COMP-5","CONTAINS","CONTENT","CONVERTING","COPY","CORRESPONDING","COUNT","DELIMITED","DELIMITER","DEPENDING","DESCENDING","DOWN","DUPLICATES","DYNAMIC","ELSE","END","ERROR","EXCEPTION","EXTEND","EXTERNAL","FALSE","FD","FILLER","FIRST","FOR","FROM","FUNCTION","GIVING","GLOBAL","GREATER","HIGH-VALUE","HIGH-VALUES","I-O","IN","INDEX","INDEXED","INITIAL","INPUT","INTO","INVALID","IS","KEY","LABEL","LEADING","LEFT","LENGTH","LESS","LINE","LINES","LOCK","LOW-VALUE","LOW-VALUES","NEGATIVE","NEXT","NO","NOT","NULL","NUMERIC","OCCURS","OF","OMITTED","ON","OPTIONAL","OR","ORDER","ORGANIZATION","OTHER","OUTPUT","OVERFLOW","PACKED-DECIMAL","PAGE","PIC","PICTURE","POINTER","POSITIVE","PROGRAM-ID","RANDOM","RECORD","RECORDS","REDEFINES","REFERENCE","RELATIVE","REMAINDER","RENAMES","REPLACE","REPLACING","RESERVE","RETURNING","RIGHT","ROUNDED","RUN","SD","SECTION","SELECT","SENTENCE","SEPARATE","SEQUENCE","SEQUENTIAL","SIGN","SIZE","SOURCE","SOURCE-COMPUTER","OBJECT-COMPUTER","SPACE","SPACES","SPECIAL-NAMES","STANDARD","STATUS","TALLYING","THAN","THEN","THROUGH","THRU","TIMES","TO","TOP","TRAILING","TRUE","UNTIL","UP","UPON","USAGE","USE","USING","VALUE","VALUES","VARYING","WHEN","WITH","WORDS","ZERO","ZEROS","ZEROES"];s.forEach(function(I){E.push({label:I,kind:e.languages.CompletionItemKind.Keyword,insertText:I+" ",detail:"COBOL keyword",range:N,sortText:"3_"+I})});var A=["ABS","ACOS","ANNUITY","ASIN","ATAN","CHAR","COS","CURRENT-DATE","DATE-OF-INTEGER","DATE-TO-YYYYMMDD","DAY-OF-INTEGER","DAY-TO-YYYYDDD","E","EXP","EXP10","FACTORIAL","INTEGER","INTEGER-OF-DATE","INTEGER-OF-DAY","INTEGER-PART","LENGTH","LOG","LOG10","LOWER-CASE","MAX","MEAN","MEDIAN","MIDRANGE","MIN","MOD","NUMVAL","NUMVAL-C","ORD","ORD-MAX","ORD-MIN","PI","PRESENT-VALUE","RANDOM","RANGE","REM","REVERSE","SIN","SQRT","STANDARD-DEVIATION","SUM","TAN","TRIM","UPPER-CASE","VARIANCE","WHEN-COMPILED","YEAR-TO-YYYY"];A.forEach(function(I){E.push({label:"FUNCTION "+I,kind:e.languages.CompletionItemKind.Function,insertText:"FUNCTION "+I+"(${1})",insertTextRules:e.languages.CompletionItemInsertTextRule.InsertAsSnippet,detail:"Intrinsic function",range:N,sortText:"4_"+I})});let l=R.getValue().split(`
`),D={};for(let I=0;I<l.length;I++){let d=l[I].trim(),S=d.match(/^(\d{1,2})\s+([\w-]+)/i);if(S&&S[2].toUpperCase()!=="FILLER"){let o=S[2].toUpperCase();D[o]||(D[o]=!0,E.push({label:S[2],kind:e.languages.CompletionItemKind.Variable,insertText:S[2],detail:"Data item (Level "+S[1]+")",documentation:d,range:N,sortText:"5_"+o}))}let L=d.match(/^([A-Z][A-Z0-9-]+)\s*\.\s*$/i);if(L&&!d.match(/DIVISION|SECTION/i)){let o=L[1].toUpperCase();D[o]||(D[o]=!0,E.push({label:L[1],kind:e.languages.CompletionItemKind.Function,insertText:L[1],detail:"Paragraph",range:N,sortText:"5_"+o}))}}return{suggestions:E}}}),e.languages.registerSignatureHelpProvider("cobol",{signatureHelpTriggerCharacters:["(",","," "],provideSignatureHelp:function(R,t){let n=R.getLineContent(t.lineNumber).substring(0,t.column-1).toUpperCase(),i={"FUNCTION LENGTH":{label:"FUNCTION LENGTH(argument)",documentation:"Returns the length of the argument in bytes.",parameters:[{label:"argument",documentation:"An alphanumeric or national data item or literal."}]},"FUNCTION TRIM":{label:"FUNCTION TRIM(argument [LEADING|TRAILING])",documentation:"Removes leading and/or trailing spaces.",parameters:[{label:"argument",documentation:"The string to trim."},{label:"LEADING|TRAILING",documentation:"Optional. Specify LEADING or TRAILING to trim only one side."}]},"FUNCTION UPPER-CASE":{label:"FUNCTION UPPER-CASE(argument)",documentation:"Converts all lowercase letters to uppercase.",parameters:[{label:"argument",documentation:"An alphanumeric data item or literal."}]},"FUNCTION LOWER-CASE":{label:"FUNCTION LOWER-CASE(argument)",documentation:"Converts all uppercase letters to lowercase.",parameters:[{label:"argument",documentation:"An alphanumeric data item or literal."}]},"FUNCTION NUMVAL":{label:"FUNCTION NUMVAL(argument)",documentation:"Converts an alphanumeric string to a numeric value.",parameters:[{label:"argument",documentation:"An alphanumeric string representing a number."}]},"FUNCTION CURRENT-DATE":{label:"FUNCTION CURRENT-DATE",documentation:"Returns a 21-character alphanumeric value: YYYYMMDDHHMMSSssZ+hhmm",parameters:[]},"FUNCTION MAX":{label:"FUNCTION MAX(argument-1 argument-2 ...)",documentation:"Returns the maximum value from the supplied arguments.",parameters:[{label:"arguments",documentation:"Two or more numeric or alphanumeric arguments."}]},"FUNCTION MIN":{label:"FUNCTION MIN(argument-1 argument-2 ...)",documentation:"Returns the minimum value from the supplied arguments.",parameters:[{label:"arguments",documentation:"Two or more numeric or alphanumeric arguments."}]},"FUNCTION MOD":{label:"FUNCTION MOD(argument-1 argument-2)",documentation:"Returns argument-1 modulo argument-2.",parameters:[{label:"argument-1",documentation:"The dividend (integer)."},{label:"argument-2",documentation:"The divisor (integer, non-zero)."}]},"FUNCTION SQRT":{label:"FUNCTION SQRT(argument)",documentation:"Returns the square root of the argument.",parameters:[{label:"argument",documentation:"A non-negative numeric value."}]}};for(let N in i)if(n.indexOf(N)!==-1){let E=i[N];return{value:{signatures:[{label:E.label,documentation:E.documentation,parameters:E.parameters}],activeSignature:0,activeParameter:0},dispose:function(){}}}return null}}),e.languages.registerFoldingRangeProvider("cobol",{provideFoldingRanges:function(R){let t=R.getLinesContent(),n=[],i=[];for(let N=0;N<t.length;N++){let E=t[N].trim().toUpperCase();if(E.match(/(IDENTIFICATION|ENVIRONMENT|DATA|PROCEDURE)\s+DIVISION/)||E.match(/(CONFIGURATION|INPUT-OUTPUT|FILE|WORKING-STORAGE|LOCAL-STORAGE|LINKAGE|SCREEN|REPORT)\s+SECTION/)){if(i.length>0){let T=i.pop();N-1>T&&n.push({start:T+1,end:N,kind:e.languages.FoldingRangeKind.Region})}i.push(N)}if(E.match(/^(IF|EVALUATE)\b/)&&!E.match(/^END-/)&&i.push(N),E.match(/^END-(IF|EVALUATE|PERFORM)\b/)&&i.length>0){let T=i.pop();n.push({start:T+1,end:N+1,kind:e.languages.FoldingRangeKind.Region})}}if(i.length>0){let N=i.pop();t.length>N+1&&n.push({start:N+1,end:t.length,kind:e.languages.FoldingRangeKind.Region})}return n}});function m(R){let t=[],n=R.getLinesContent(),i=!1;for(let r=0;r<n.length;r++){let a=n[r],s=a.trim().toUpperCase();s.match(/PROCEDURE\s+DIVISION/)&&(i=!0),s.match(/(DIVISION|SECTION)\s*$/)&&!s.endsWith(".")&&t.push({severity:e.MarkerSeverity.Error,message:"Missing period after DIVISION/SECTION declaration.",startLineNumber:r+1,startColumn:1,endLineNumber:r+1,endColumn:a.length+1}),s.match(/\bPREFORM\b/)&&t.push({severity:e.MarkerSeverity.Warning,message:"Did you mean PERFORM?",startLineNumber:r+1,startColumn:a.toUpperCase().indexOf("PREFORM")+1,endLineNumber:r+1,endColumn:a.toUpperCase().indexOf("PREFORM")+8}),s.match(/\bDSPLAY\b/)&&t.push({severity:e.MarkerSeverity.Warning,message:"Did you mean DISPLAY?",startLineNumber:r+1,startColumn:a.toUpperCase().indexOf("DSPLAY")+1,endLineNumber:r+1,endColumn:a.toUpperCase().indexOf("DSPLAY")+7}),s.match(/\bMOVVE\b/)&&t.push({severity:e.MarkerSeverity.Warning,message:"Did you mean MOVE?",startLineNumber:r+1,startColumn:a.toUpperCase().indexOf("MOVVE")+1,endLineNumber:r+1,endColumn:a.toUpperCase().indexOf("MOVVE")+6}),a.length>80&&t.push({severity:e.MarkerSeverity.Info,message:"Line exceeds 80 characters (column limit in traditional COBOL fixed format).",startLineNumber:r+1,startColumn:81,endLineNumber:r+1,endColumn:a.length+1})}let N={IF:"END-IF",EVALUATE:"END-EVALUATE"},E=0,T=0;for(let r=0;r<n.length;r++){let a=n[r].trim().toUpperCase();a.match(/\bIF\b/)&&!a.match(/\bEND-IF\b/)&&E++,a.match(/\bEND-IF\b/)&&E--,a.match(/\bEVALUATE\b/)&&!a.match(/\bEND-EVALUATE\b/)&&T++,a.match(/\bEND-EVALUATE\b/)&&T--}E>0&&t.push({severity:e.MarkerSeverity.Warning,message:E+" unmatched IF statement(s) \u2014 missing END-IF.",startLineNumber:1,startColumn:1,endLineNumber:1,endColumn:2}),T>0&&t.push({severity:e.MarkerSeverity.Warning,message:T+" unmatched EVALUATE statement(s) \u2014 missing END-EVALUATE.",startLineNumber:1,startColumn:1,endLineNumber:1,endColumn:2}),e.editor.setModelMarkers(R,"cobol",t)}e.languages.registerRenameProvider("cobol",{provideRenameEdits:function(R,t,n){let i=R.getWordAtPosition(t);if(!i)return null;let N=i.word,E=a=>a.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),T=[],r=new RegExp("\\b"+E(N)+"\\b","g");return R.getLinesContent().forEach((a,s)=>{r.lastIndex=0;let A;for(;(A=r.exec(a))!==null;)T.push({resource:R.uri,versionId:R.getVersionId(),textEdit:{range:new e.Range(s+1,A.index+1,s+1,A.index+1+N.length),text:n}})}),{edits:T}},resolveRenameLocation:function(R,t){let n=R.getWordAtPosition(t);return n?{range:new e.Range(t.lineNumber,n.startColumn,t.lineNumber,n.endColumn),text:n.word}:{rejectReason:"Cannot rename this element."}}})};export{c as default};
