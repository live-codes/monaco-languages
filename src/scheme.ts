...
export default (monaco: typeof Monaco) => {
  ...
  const resolveBinding = (
    ...
    // Names bound by a local binding form (`let`/`lambda` and friends) are
    // scoped to that form; top-level definitions stay document-wide.
    const declaration = new RegExp(
      "\\((?:let|let\\*|letrec)\\s+\\(\\s*\\(?\\s*" +
        esc(name) +
        "\\b|\\((?:lambda|fn)\\s+\\(\\s*" +
        esc(name) +
        "\\b",
  ...
  monaco.languages.registerRenameProvider("scheme", {
