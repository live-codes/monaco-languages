...
export default (monaco: typeof Monaco) => {
  ...
  monaco.languages.registerRenameProvider("lua", {
    provideRenameEdits: function (model, position, newName) {
      ...
      // Lua blocks: a keyword line opens a scope that its `end` (or `until`)
      // closes, so each function/loop body is its own scope.
      type Scope = {
        start: number;
        end: number;
        line: number;
      ...
      const isOpener = (code: string) =>
        /^(?:local\s+)?(?:function|if|for|while|do|repeat)\b/.test(code) ||
        /(?:\bthen\b|\bdo\b)\s*$/.test(code);
      const isCloser = (code: string) => /^(?:end|until)\b/.test(code);
      const scopes: Scope[] = [];
      const open: Scope[] = [];
      for (let i = 0; i < lines.length; i++) {
      ...
      // Local declarations: `local NAME`, loop variables and parameters.
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = declaration.exec(lines[i])) !== null) {
          ...
          // such as `print(x)` is not mistaken for a declaration.
