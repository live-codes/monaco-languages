...
export default (monaco: typeof Monaco) => {
  ...
  monaco.languages.registerRenameProvider("elixir", {
    provideRenameEdits: function (model, position, newName) {
      ...
      // Elixir blocks: a keyword line opens a scope that its `end` closes. A
      // one-liner such as `def f(x), do: x` does not open a block.
      type Scope = {
        start: number;
        end: number;
        line: number;
      ...
      const isOpener = (code: string) =>
        /^(?:defmodule|defprotocol|defimpl|def|defp|defmacro|defmacrop|if|unless|case|cond|for|while|try|receive|fn|with|quote)\b/.test(
          code,
        ) && !/\bdo:\s/.test(code);
      const isCloser = (code: string) => /^end\b/.test(code);
      const scopes: Scope[] = [];
      const open: Scope[] = [];
      for (let i = 0; i < lines.length; i++) {
      ...
      // Local declarations: `NAME = …` bindings and parameters.
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = declaration.exec(lines[i])) !== null) {
          ...
          // such as `IO.puts(x)` is not mistaken for a declaration.
