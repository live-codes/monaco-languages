...
export default (monaco: typeof Monaco) => {
  ...
  monaco.languages.registerRenameProvider("julia", {
    provideRenameEdits: function (model, position, newName) {
      ...
      // Julia blocks: a keyword line opens a scope that its `end` closes, so
      // each function/module/loop body is its own scope.
      type Scope = {
        start: number;
        end: number;
        line: number;
      ...
      const isOpener = (code: string) =>
        /^(?:function|if|for|while|begin|let|quote|try|struct|mutable\s+struct|module|baremodule|macro)\b/.test(
          code,
        ) || /\bdo\s*$/.test(code);
      ...
      // Local declarations: local/global/const bindings, for-loop variables,
      // assignments and parameters.
      const declaration = new RegExp(
        "\\b(?:local|global|const)\\s+" +
          esc(name) +
          "\\b|\\bfor\\s+" +
          esc(name) +
          "\\b|\\b" +
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = declaration.exec(lines[i])) !== null) {
          ...
          // such as `hypot(total.x)` is not mistaken for a declaration.
