...
export default (monaco: typeof Monaco) => {
  ...
  const resolveBinding = (
    ...
    // Local declarations: var/const bindings and parameters.
  ...
  monaco.languages.registerRenameProvider(ZIG_LANG_ID, {
    provideRenameEdits: function (model, position, newName) {
      const word = model.getWordAtPosition(position);
      if (!word) return null;
      const name = word.word;
      if (ZIG_KEYWORDS.includes(name)) return null;
    ...
    resolveRenameLocation: function (model, position) {
      ...
      if (ZIG_KEYWORDS.includes(word.word))
