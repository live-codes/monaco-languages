...
export default (monaco: typeof Monaco) => {
  ...
  monaco.languages.registerRenameProvider("java", {
    provideRenameEdits: function (model, position, newName) {
      ...
      // Local declarations: typed locals, for-each bindings and parameters.
      // A method declaration is excluded because its name is followed by "(".
      const declaration = new RegExp(
        "\\b(?:final\\s+)?(?:[A-Z][A-Za-z0-9_]*|int|long|short|byte|char|float|double|boolean|var|String)(?:\\s*<[^;{}()]*>)?(?:\\s*\\[\\s*\\])*\\s+" +
          esc(name) +
          "\\s*[=;:,\\)\\[\\]]",
        "g",
      );
