...
export default (monaco: typeof Monaco) => {
  ...
  const resolveBinding = (
    ...
    // First scope opening at or after `offset` (a function's body). A call like
    // `f(x)` is not a signature: anything past a statement boundary is rejected
    // so it cannot be mistaken for a parameter list.
    ...
    // Local declarations: `x <- …`, `x <<- …`, `x = …` and parameters.
  ...
  monaco.languages.registerRenameProvider("r", {
