...
const tokens: languages.IMonarchLanguage = {
  ...
  tokenizer: {
    root: [
      ...
      ], // Catch-all for math symbols if not explicitly listed
...
const completions: languages.CompletionItemProvider = {
  provideCompletionItems: (model, position) => {
    ...
    const suggestions = [
      ...
      ...builtins.map((b) =>
        createItem(b, 18 /* Function */, b, "Built-in Function"),
      ),
      ...snippets.map((s) => ({
        label: s.label,
        kind: 27 /* Snippet */,
...
export default (monaco = (window as any).monaco) => {
  ...
  // ─── Rename Provider ────────────────────────────────────────────────
  // MiniZinc declarations are model-global, so every occurrence is renamed.
