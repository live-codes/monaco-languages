...
export default (monaco: typeof Monaco) => {
  ...
  monaco.languages.registerRenameProvider("vbnet", {
    provideRenameEdits: function (model, position, newName) {
      ...
      // VB blocks: a keyword line opens a scope that `End …` closes. A one-line
      // `If … Then …` does not open a block.
      type Scope = {
        start: number;
        end: number;
        line: number;
      ...
      const isOpener = (code: string) =>
        /^(?:sub|function|if|for|while|do|select|try|class|module|structure|using|with|namespace|property|get|set|event|enum|interface)\b/i.test(
          code,
        ) && !/^if\b.*\bthen\b\s*\S/i.test(code);
      const isCloser = (code: string) =>
        /^(?:end|next|loop|wend)\b/i.test(code);
      const scopes: Scope[] = [];
      const open: Scope[] = [];
      for (let i = 0; i < lines.length; i++) {
      ...
      // Local declarations: Dim/Const bindings and parameters (VB is
      // case-insensitive, so the keywords are matched case-insensitively).
      const declaration = new RegExp(
        "\\b(?:dim|const|static)\\s+" +
          esc(name) +
          "\\b|(?:[(,]\\s*)" +
          esc(name) +
          "\\s*[,:)]",
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = declaration.exec(lines[i])) !== null) {
          ...
          const isParam =
            /^[(,|[{[]/.test(m[0]) || /[(,{[]$/.test(before);
          const owner = enclosing(at(i, m.index));
          // A parameter list only declares inside the block it heads, so a call
          // such as `WriteLine(x)` is not mistaken for a declaration.
          if (isParam && (!owner || owner.line !== i || owner.start > at(i, m.index)))
            continue;
