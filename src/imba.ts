...
function denter(indent, outdent, stay, o = {}) {
  ...
  let cases = {
    ...
    "$1==$S2": {
      cases: {
        "$1==$S6": stay,
        "@default": { token: "@rematch", switchTo: "@*$1" },
      },
    },
    "@default": outdent,
  };
  ...
  if (o.comment) {
    let clones = {};
    for (
      let iφ = 0, keysφ = Object.keys(cases), lφ = keysφ.length, k, v;
      iφ < lφ;
      iφ++
    ) {
      k = keysφ[iφ];
...
const states = {
  ...
  import_: [
    ...
    [
      ...
      [
        "keyword.import",
        "keyword.type",
        "@>import_body&-_imports=decl-import/part",
      ],
    ],
    [
      /(import)@B/,
  ...
  export_body: [
    denter(null, -1, 0),
    [
      /(\*)(\s+as\s+)(@esmIdentifier)/,
      ["keyword.star", "keyword.as", "identifier.const.export"],
    ],
    [
      /(@esmIdentifier)(\s+as\s+)(default)/,
      ["alias", "keyword.as", "alias.default"],
    ],
    [
  ...
  member_: [
    ...
    [
      /(def|get|set)(\s)(\[)/,
      ["keyword.$1", "white.entity", "$$", "@>def_dynamic_name/$1"],
    ],
  ],
  ...
  func_: [
    ...
    [
      ...
      [
        "keyword.$1",
        "white.entity",
        "entity.name.function.decl-const-func",
        "@>def_params&$1/$1",
      ],
    ],
  ],
  ...
  inline_var_: [
    [
      /(const|let)(?=\s[\[\{\$a-zA-Z]|\s*$)/,
      ["keyword.$1", "@inline_var_body=decl-$1"],
    ],
  ],
    ...
    [
      /\{/,
      {
        cases: {
          "$F=='": "string",
          "@default": {
            token: "string.bracket.open",
            next: "@interpolation_body",
          },
        },
      },
    ],
  ...
  sel_: [
    ...
    [
      /(\$)((?:@id)?)/,
      ["style.selector.reference.prefix", "style.selector.reference"],
    ],
  ...
  css_selector: [
    denter({ switchTo: "@css_props" }, -1, {
      token: "@rematch",
      switchTo: "@css_props&_props",
    }),
    [/(\}|\)|\])/, "@rematch", "@pop"],
  ...
  css_property: [
    ...
    [
      /(\@+|\.+)(@id\-?)/,
      ["style.property.modifier.start", "style.property.modifier"],
    ],
    ...
    [
      /\s*([\:]\s*)/,
      "style.property.operator",
      { switchTo: "@>css_value&_stylevalue" },
    ],
  ],
  ...
  css_value_: [
    ...
    [
      ...
      [
        "style.property.name",
        "style.property.modifier.prefix",
    ...
    [
      /\{/,
      "delimiter.style.curly.open",
      "@css_interpolation&-_styleinterpolation",
    ],
    [/(@id)/, "style.value"],
  ],
  ...
  _tag: [
    ...
    [
      /\./,
      {
        cases: {
          "$/==event": {
            token: "tag.event-modifier.start",
            switchTo: "@/event-modifier",
          },
          "$/==event-modifier": {
            token: "tag.event-modifier.start",
            switchTo: "@/event-modifier",
          ...
          "$/==modifier": {
            token: "tag.modifier.start",
            switchTo: "@/modifier",
          },
          "$/==rule": {
            token: "tag.rule-modifier.start",
            switchTo: "@/rule-modifier",
          },
          "$/==rule-modifier": {
            token: "tag.rule-modifier.start",
            switchTo: "@/rule-modifier",
  ...
  _regexp: [
    ...
    [
      ...
      [
        "regexp.escape.control",
        { token: "regexp.escape.control", next: "@_regexrange" },
      ],
    ],
    ...
    [
      /(\/)(\w+)/,
      [
        { token: "regexp.slash.close" },
        { token: "regexp.flags", next: "@pop" },
      ],
    ],
...
function rewriteΞactions(actions, add) {
  ...
  if (actions && actions.token != undefined) {
    ...
    let cases = {};
    for (
      let oφ = actions.cases,
        iφ3 = 0,
        keysφ2 = Object.keys(oφ),
        lφ2 = keysφ2.length,
        k,
        v;
      iφ3 < lφ2;
    ...
    let curr = null;
    for (
      let i = 0, itemsφ2 = iter$__(actions), lenφ2 = itemsφ2.length;
      i < lenφ2;
      i++
    ) {
  ...
  if (actions instanceof Array) {
    for (
      let i = 0, itemsφ3 = iter$__(actions), lenφ3 = itemsφ3.length;
      i < lenφ3;
      i++
    ) {
...
for (
  let iφ4 = 0, keysφ3 = Object.keys(states), lφ3 = keysφ3.length, key, rules;
  iφ4 < lφ3;
  iφ4++
) {
  key = keysφ3[iφ4];
...
const tokens = {
  ...
  propertyPath:
    /(?:[A-Za-z_\$][A-Za-z\d\-\_\$]*\.)?(?:[A-Za-z_\$][A-Za-z\d\-\_\$]*)/,
  ...
  // cssModifier: /\@[\w\-\$]+/
  cssPropertyKey2:
    /(?:@cssPropertyName(?:@cssModifier)*|@cssModifier+)(?:\s*\:)/,
  cssUpModifier: /\.\.[\w\-\$]+/,
  cssIsModifier: /\.[\w\-\$]+/,
...
export default (monaco = (window as any).monaco) => {
  ...
  monaco.languages.registerRenameProvider(lang, {
    provideRenameEdits: function (model, position, newName) {
      ...
      // Imba is indentation based: a line whose next line is indented further
      ...
      // Local declarations: var/let/const bindings.
      const declaration = new RegExp(
        "\\b(?:var|let|const)\\s+" + esc(name) + "\\b",
        "g",
      );
