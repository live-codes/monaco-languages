...
export default (monaco: typeof Monaco) => {
  ...
  monaco.languages.registerRenameProvider(LANG_ID, {
    provideRenameEdits: function (model, position, newName) {
      ...
      // Lean blocks are indentation based: a line whose next line is indented
      // further heads a block that runs until the indentation drops back. The
      // block starts on the body's first line, so a top-level declaration stays
      // at document level.
      type Scope = {
        start: number;
        end: number;
      ...
      // Local declarations: def/theorem/lemma/… and let bindings.
      const declaration = new RegExp(
        "\\b(?:def|theorem|lemma|abbrev|opaque|structure|inductive|class)\\s+" +
          escapeRegex(name) +
          "\\b|\\blet\\s+" +
          escapeRegex(name) +
          "\\b",
